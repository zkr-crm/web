	...
	function MsgCenterCtrl($scope, $sce, $rootScope, $uibModal, $state,
		...
		$scope.getEmployeeId = function() {
			...
			HttpService.linkHttp(opts).then(function(response) {
				...
				// 获取登录用户的员工ID
				$scope.RemindSearch.employeeId = userInfo.employeeId;
				// 获取当前用户的所有未读站内提醒
				$scope.getUnReadRemind();

				// 获取登录用户的员工ID
				$scope.MailSearch.employeeId = userInfo.employeeId;
				// 获取当前用户的所有未读站内提醒
				$scope.getUnReadMail();
			});
		};

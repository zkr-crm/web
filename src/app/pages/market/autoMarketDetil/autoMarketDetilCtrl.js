(function() {
	...
	angular.module('BlurAdmin.pages.market.autoMarketDetil').controller('autoMarketDetilCtrl', autoMarketDetilCtrl);

	/** @ngInject */
	function autoMarketDetilCtrl($scope, $state, $stateParams, $uibModal, Alert, EnumType) {
		...
		for (var i = 0; i < 12; i++) {
			...
			personObj.singTime = "参加时间-" + i;
			personObj.allCount = "领取优惠券数-" + i;
			personObj.usedCount = "使用优惠券数-" + i;
		...
		$scope.sendMsg = function() {
			...
			$uibModal.open({
				animation : true,
				backdrop : 'static',
				templateUrl : 'app/pages/market/autoMarketDetil/popUps/sendMsg.html',
				size : 'midle-900',
		...
		$scope.entryFeedBack = function() {
			$uibModal.open({
				animation : true,
				backdrop : 'static',
				templateUrl : 'app/pages/market/autoMarketDetil/popUps/entryFeedBack.html',
				size : 'midle-900',
		...
		$scope.addTask = function() {
			var modalInstance = $uibModal.open({
				animation : true,
				templateUrl : 'app/pages/market/autoMarketDetil/popUps/addTask.html',
		...
		$scope.getStrategy = function() {
			...
			var modalInstance = $uibModal.open({
				animation : true,
				backdrop : 'static',
				templateUrl : 'app/pages/market/autoMarketDetil/popUps/getStrategy.html',
				size : 'midle-900',
				controller : 'getStrategyCtrl',
				scope : $scope,
				resolve : {
					items : function() {
						return $scope.items;
			...
			modalInstance.result.then(function(result) {
				$scope.activInfo.strategyId = result.strategyId; // 策略ID
				$scope.activInfo.strategyName = result.strategyName; // 策略名称

(function () {
  ...
  angular.module('BlurAdmin.pages.cusService.custRetVisit.retVisitAllot')
      .controller('custRetVisitDetCtrl', custRetVisitDetCtrl);
  
  function custRetVisitDetCtrl($scope,$state,$stateParams,$uibModal, HttpService, EnumType,$filter, Alert,$sce,$rootScope) {
	$scope.touchItem = $stateParams.touchItem;
	if ($scope.touchItem == undefined || $scope.touchItem.custNo == undefined) {
		return;
	}
	$scope.retVisitSts = $scope.touchItem.retVisitSts;
    ...
    var stepInt = 0;
    if ($scope.retVisitSts == undefined || $scope.retVisitSts == '') {
    	stepInt = 0;
    } else {
    	stepInt = $scope.retVisitSts + 1;
    }

(function () {
  ...
  angular.module('BlurAdmin.pages.cusService.custConsult.consultAllot')
      .controller('custConsultDetCtrl', custConsultDetCtrl);
  
  function custConsultDetCtrl($scope,$state,$stateParams,$uibModal, HttpService, EnumType,$filter, Alert,$sce,$rootScope) {
	$scope.touchItem = $stateParams.touchItem;

	if ($scope.touchItem == undefined || $scope.touchItem.custNo == undefined) {
		return;
	}
	$scope.consultSts = $scope.touchItem.consultSts;
    ...
    var stepInt = 0;
    if ($scope.consultSts == undefined || $scope.consultSts == '') {
    	stepInt = 0;
    } else {
    	stepInt = $scope.consultSts + 1;
    }

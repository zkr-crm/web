(function() {
    ...
    angular.module('mgrcenter.engineManage.baserule.modify').controller(
        'modifyBaseruleCtrl', modifyBaseruleCtrl);
    /** @ngInject */
    function modifyBaseruleCtrl($scope,$log,$state,$stateParams, toastr, toastrConfig,$compile, $uibModal, $filter, $timeout, $http,
        ...
        $scope.baseRuleAdd = {
            ...
            loadFactor: function () {
               ...
               $scope.isTreeShow = false;
                HttpService.linkHttp(opts).then(function(response) {
                    ...
                    $scope.treeItemList = ztreeDefaultData;
                   ...
                   // debugger ;
                    ...
                    response.data.forEach(function(value,index,array){
                        ...
                        $scope.treeItemList.push(treeItem)
                    ...
                    // $.fn.zTree.init($("#factorTree"), setting, $scope.treeItemList);
            ...
            doChangeFactorName:function(id){
                ...
                if (valueType == "SELECT"){
                    ...
                    HttpService.linkHttp(opts).then(function(response) {
                        ...
                        var select = "<select class='form-control'  id='value_"+index+"' style='width: 180px;' name='value_"+index+"' onchange='updateRuleTranslation()'>";
                        var selectOption = "";
                        response.data.forEach(function(value,index,array){
                    ...
                    valueHtml = "<input type='text' class='form-control input-sm' style='width: 180px;'  name='value_"+index+"' id='value_"+index+"' placeholder='参考值' onblur='blurCheck(this)'>";
            ...
            addRow: function (rowBtn, optVal) {
                ...
                var row = "<tr id='row_"+index+"'>"
                    ...
                    + '<button type="button" class="btn btn-sm btn-default dropdown-toggle drag-item" onclick="addExp(this)"><span exp="R'+index+'">R'+index+'</span></button>'
                    + "</td>"
                    + "<td>"
                    ...
                    + "</select></td>"
                    + "<td>"
                    + "<select class='form-control'  id='operate_"+index+"' style='width: 180px;' name='operate_"+index+"' onchange='updateRuleTranslation()'>"
                    + $scope.baseRuleAdd.htmlSegment['LOGICAL']
                    + "</select></td>"
                    + "<td id='valueTd_"+index+"'>"
                    + "<select class='form-control' id='value_"+index+"' style='width: 180px;' name='value_"+index+"' onchange='updateRuleTranslation()'>"
            ...
            deleteRow: function (rowBtn) {
                ...
                $scope.baseRuleAdd.updateTranslation();
            ...
            addExistRow: function (rowObj) {
                ...
                var row = "<tr id='row_"+$scope.baseRuleAdd.index+"'>"
                    + "<td align='center' id='order_"+$scope.baseRuleAdd.index+"'>"
                    + "<button type='button' class='btn btn-sm btn-default dropdown-toggle drag-item' onclick='addExp(this)'><span exp='R"+$scope.baseRuleAdd.index+"'>R"+$scope.baseRuleAdd.index+"</span></button>"
                    + "</td>"
                    + "<td>"
                    + "<select class='form-control' ng-model='factorName_"+$scope.baseRuleAdd.index+"'  id='factorName_"+$scope.baseRuleAdd.index+"' style='width: 180px;' name='factorName_"+$scope.baseRuleAdd.index+"' ng-change='baseRuleAdd.doChangeFactorName(factorName_"+$scope.baseRuleAdd.index+")'>"
                    + $("#factorName_select").html()
                    + "</select></td>"
                    + "<td>"
                    + "<select class='form-control' id='operate_"+$scope.baseRuleAdd.index+"' style='width: 180px;' name='operate_"+$scope.baseRuleAdd.index+"'  onchange='updateRuleTranslation()'>"
                    + $scope.baseRuleAdd.htmlSegment['LOGICAL']
                    + "</select></td>"
                    + "<td  id='valueTd_"+$scope.baseRuleAdd.index+"'>"
                    + "<select class='form-control'  id='value_"+$scope.baseRuleAdd.index+"' style='width: 180px;' name='value_"+$scope.baseRuleAdd.index+"' onchange='updateRuleTranslation()'>"
                    + "<option value='\"whiteList\"'>白名单</option>"
                    + "<option value='\"grayList\"'>灰名单</option>"
                    + "<option value='\"blackList\"'>黑名单</option>"
                    + "</select>"
                    + "</td>"
                    + "<td>"
                    + "<button type=\"button\" class=\"btn btn-danger btn-icon ion-trash-a\" ng-click=\"baseRuleAdd.deleteRow("+$scope.baseRuleAdd.index+")\"></button>"
                    + "<button type='button' class='close hide' ng-click='baseRuleAdd.addRow(this)'>"
                    + "<span class='glyphicon glyphicon-plus' aria-hidden='true'></span>"
                    + "</button>"
                    + "</td>"
                ...
                var $row = $compile(row)($scope);
                $("#factorTable tbody").append($row);
                //修改因子名
                $("#factorName_"+$scope.baseRuleAdd.index).val(rowObj.factor_id);
                //触发this.doChangeFactorName()
                $scope.baseRuleAdd.doChangeFactorName("factorName_"+$scope.baseRuleAdd.index);
                //修改操作符的值
                $("#operate_"+$scope.baseRuleAdd.index).val(rowObj.operate);
                // var operName = "operate_" + +$scope.baseRuleAdd.index
                // $scope[operName] = rowObj.operate;

                //修改因子的值
                $("#value_"+$scope.baseRuleAdd.index).val(rowObj.factor_value);

                var factorSelect = $("#factorName_"+$scope.baseRuleAdd.index);
                var factorOptChecked = factorSelect.find("option:selected");
                factorSelect.after("<input type='hidden' id='"+factorSelect.attr("id")+"' name='"+factorSelect.attr("name")+"' value='"+factorOptChecked.attr("value")+"' operateName='"+factorOptChecked.attr("name")+"' operateType='"+factorOptChecked.attr("operateType")+"' valueType='"+factorOptChecked.attr("valueType")+"' />"
                    + "<input type='text' class='form-control' value='"+$.trim(factorOptChecked.text())+"'  title='"+$.trim(factorOptChecked.text())+"' style=\"color:#6a6a6a\" readonly />").remove();
                ...
                //     } else {
                        ...
                        HttpService.linkHttp(opts).then(function(response) {
                            ...
                            if (response.data=="0"){
                                ...
                                //校验所选因子是否全部应用到表达式中
                                $scope.baseRuleObj.ruleName = ruleName ;
                                $scope.baseRuleObj.expression = expression ;
                                $scope.baseRuleObj.remark = $("#remark").val() ;

                                var opts = {};
                                opts.url = '/crm/manage/engine/updBaseRule';
                                opts.method = 'PUT';
                                opts.params = $scope.baseRuleObj;
                                opts.data = factorArr;
                                HttpService.linkHttp(opts).then(function(response) {
                                    // Alert.success("请求成功");
        ...
        $scope.onDblClick = function ( treeNode) {
            // alert(treeNode.pId);
            // // alert(selected.node.id + ' ==> ' + selected.node.text);
            var nodeItem = treeNode.basicTree.jstree(true).get_selected([true])[0]
            if (nodeItem.parent != "#" && nodeItem.parent != null) {
                var success = $scope.baseRuleAdd.addRow($("#factorTable tr:last > td:last > button:last"), nodeItem.id);
                if (success) {
                    itemDragInit($("#factorTable .drag-item:last"));
                }
                return false;
        ...
        var ztreeDefaultData = [
            {id: "n1", parent: "#", text: "基础因子",type:"folder", state:{'opened':true}},
            {id: "n2", parent: "#", text: "复杂因子", type:"folder", state:{'opened':true}}
        ];
        ...
        $scope.updateInit = function () {
            ...
            console.log($stateParams.baseRule);
            console.log($stateParams.baseFactorRels);
            console.log($stateParams.isUsed);

            $scope.baseRuleObj = $stateParams.baseRule;
            // $scope.baseFactorRelsObj = eval ("(" + $stateParams.baseFactorRels+ ")") ;
            $scope.baseFactorRelsObj = $stateParams.baseFactorRels ;
            $scope.isUsedParam = $stateParams.isUsed;

            if ($scope.baseRuleObj != null) {
                $scope.baseRuleAdd.compileExp($scope.baseRuleObj.expression);

                if ($scope.baseFactorRelsObj != null) {
                    $scope.baseFactorRelsObj.forEach(function (value) {
                        $scope.baseRuleAdd.addExistRow(value);
                    })
                }
            }else {
                $state.go('mgrcenter.engineManage.baserule',{});

(function() {
	...
	angular.module('BlurAdmin.pages.mgrcenter.engineManage.baserule').controller(
			'baseruleCtrl', baseruleCtrl);
	/** @ngInject */
	function baseruleCtrl($scope,$state, toastr, toastrConfig, $uibModal, $filter, $timeout, $http,
		...
		$scope.addBaseRule = function() {
            ...
            $state.go('mgrcenter.addBaserule',{});
            // $uibModal
				// 	.open({
				// 		animation : true,
				// 		backdrop : 'static',
        ...
        $scope.viewRule = function(item) {
            ...
            $state.go('mgrcenter.viewBaserule', {
                'baseRule': item
            });
		...
		$scope.updRule = function(item) {
            ...
            var opts = {};
            opts.url = '/crm/manage/engine/getBaseRuleToUpdate';
            opts.method = 'GET';
            opts.params = {'baseRuleId':item.baseRuleId};
            HttpService.linkHttp(opts).then(function(response) {
                // console.log("请求成功");
		...
		$scope.removeRule = function(item) {
			...
			Alert.confirm("确定删除？").then(function() {
				...
				opts.method = 'POST';
				opts.data = [{'baseRuleId' :item.baseRuleId}]
				HttpService.linkHttp(opts).then(function(response) {

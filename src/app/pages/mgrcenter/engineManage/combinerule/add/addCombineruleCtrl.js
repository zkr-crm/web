(function() {
    ...
    angular.module('mgrcenter.engineManage.combinerule.add').controller(
        'addCombineruleCtrl', addCombineruleCtrl);
    /** @ngInject */
    function addCombineruleCtrl($scope,$sce,$log,$state, toastr, toastrConfig,$compile, $uibModal, $filter, $timeout, $http,
        ...
        $scope.combineRuleAdd = {
            ...
            addRow:function(items){
                ...
                if(rowid&&!exists){
                    ...
                    var row = "<tr id='"+rowid+"' exp='"+rowObject.expression+"' order='BR"+index+"'>"
                        ...
                        + "<button type='button' style='margin-right:5px' class='btn btn-info btn-icon ion-eye' ng-click='combineRuleAdd.viewBaseRule(\""+rowid+"\")'></button>"
                        + "<button type='button' class='btn btn-danger btn-icon ion-trash-a' ng-click='combineRuleAdd.deleteRow(\""+rowid+"\")'></button>"
                        + "</td>"
                        + "</tr>";
            ...
            addExistRow: function (rowObj) {
                var row = "<tr id='"+rowObj.base_rule_id+"' exp='"+rowObj.base_rule_order+"' order='"+rowObj.base_rule_order+"'>"
                    + "<td align='center'>"
                    + "<button type='button' class='btn btn-sm btn-default dropdown-toggle drag-item' onclick='combineRuleAdd.addExp(this)'>" +
                    ...
                    + "<td align='center' class='text-danger'>"
                    + "<button type='button' class='btn btn-sm btn-primary' onclick='javascript:combineRuleAdd.viewBaseRule(\""+rowObj.base_rule_id+"\")'><!--<span class='glyphicon glyphicon-remove' aria-hidden='true'></span>-->查看</button>"
                    + "<button type='button' class='btn btn-sm btn-danger' onclick='javascript:combineRuleAdd.deleteRow(\""+rowObj.base_rule_id+"\")'><!--<span class='glyphicon glyphicon-remove' aria-hidden='true'></span>-->删除</button>"
                    + "</tr>";
                $("#baseRuleTable").append(row);
                this.itemDragInit($("#baseRuleTable .drag-item:last"));
                this.index++;
            },
            /**
             * 将表达式解析成html代码
                    ...
                    if(response.data != "0"){// 校验名称是否重复
                        ...
                        HttpService.linkHttp(opts).then(function(response) {
                            ...
                            if (response.data=="0"){
                                ...
                                var hasUnUsed = false;
                                var unUsedBR = [];
                                $.each($("#baseRuleTable tr:gt(1)"), function (i, domEle) {
            ...
            update: function () {
                ...
                expression =  combineRuleAdd.addBracket(expression);
            ...
            formatters:{
                formatOperate: function (cellvalue, options, rowObject) {
                    return "<button type='button' class='btn btn-sm btn-primary' " +
                        "onclick='javascript:combineRuleAdd.closeBRWindow(\""+rowObject.id+"\")'>" +

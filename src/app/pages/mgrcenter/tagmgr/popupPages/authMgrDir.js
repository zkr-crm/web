(function () {
  ...
  angular.module('BlurAdmin.pages.mgrcenter.tagMgr')
      .directive('deptZtree', function($filter, $timeout, $http, HttpService) {
          // 将对象return出去
          return{
...
//                      saveTag.tagScopeVal = v;
                  ...
                  function onBodyDown(event) {
                      if (!(event.target.id == "menuBtn" || event.target.id == "tagScopeVal" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length>0)) {
                          hideMenu();
                      }
                  }

(function () {
    ...
    /** @ngInject */
    function addTagTypeCtrl($scope, $filter, $uibModal, $timeout, toastr, Alert, HttpService, selected) {
        //用户信息对象
        $scope.saveType = {
            "tagTypeName": "",
            "superTagTypeId": "",
        ...
        HttpService.linkHttp(optsForST).then(function (response) {
            angular.forEach(response.data, function (item) {
                ...
                if (item.tagTypeId == selected) {
                     $scope.saveType.superTagTypeId = {label: item.tagTypeName, value: item.tagTypeId}
                }
            })

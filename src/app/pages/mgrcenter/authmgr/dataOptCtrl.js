(function() {
	...
	function dataOptCtrl($scope, $filter, $uibModal, $timeout, HttpService,
        ...
        $scope.clickFieldBtn = function(fieldCode){    // 单机field按钮 根据field.fieldCode查询type 列表
            ...
            if($scope.beforefieldCode != '') {  // 当第一次单击field按钮时 缓存中不保存操作行为数据
                if($scope.resDataType.value.id != 'default' || $scope.resDataType.value.id !='') { // 判断beforeRuleode值是非空时才做map数据保存操作
                    ...
                    console.log('01 ----------- put data --------------')
                    console.log($scope.beforefieldCode + ' ==> ' + $scope.resDataType.value.id);
                    console.log('01 ----------- put data --------------')
                    console.log(' ')
                } else {
            ...
            console.log('02 before = '+$scope.beforefieldCode);
            console.log('02 current = '+$scope.currentFieldCode);
            console.log(' ')
            ...
            HttpService.linkHttp(opts).then(function(response) {
                ...
                if(typeof($scope.fieldMap.get($scope.currentFieldCode)) != "undefined"){ // map非空时走map 否则走db
                    ...
                    console.log('03 -----------get data map--------------')
                    var initLuleCode = bindData[0];
                    console.log($scope.currentFieldCode + ' ==> ' + initLuleCode);
                    console.log('03 -----------get data map--------------')
                    console.log(' ')
                    var initLuleName = '' ;
        ...
        function parseFieldMap2Obj() {
            // console.log('xxxxxxxxxx  fieldMap  xxxxxxxxxxxxxxxx')
            // console.log($scope.fieldMap);
            ...
            console.log('xxxxxxxxxx  dataAuthObjList  xxxxxxxxxxxxxxxx')
            console.log(dataAuthObjList);

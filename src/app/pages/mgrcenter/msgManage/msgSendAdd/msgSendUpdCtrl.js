(function() {
    ...
    angular.module('BlurAdmin.pages.mgrcenter.msgManage').controller('msgSendUpdCtrl', msgSendUpdCtrl);
    /** @ngInject */
    function msgSendUpdCtrl($scope, $state, $filter,HttpService,EnumType,Alert,$uibModal) {

        $scope.msgSendDefDts = $scope.msg;
...
//        $scope.radioSendTyp.select = "norecycle";
        ...
        function genSetParm() {
            ...
            if($scope.radioOptions.select == "direct"){
                ...
                if($scope.msgSendDefDts.fixHour ==null || $scope.msgSendDefDts.fixHour ==''){
                   ...
                   var startDate = new Date($scope.msgSendDefDts.fixHour);
                   $scope.msgSendDefDts.fixHour = $filter('date')(startDate, 'HH:mm');
                    // yyyy-MM-dd HH:mm:ss
                    $scope.msgSendDefDts.fixTime =  $scope.msgSendDefDts.fixTime + " " +$scope.msgSendDefDts.fixHour;
                }
            ...
            //实体对象ZcmMsgSendDef
            var opts = {};
            opts.url = '/crm/manage/msgmng/modMsg';
            opts.method = 'PUT';
		...
		// 初始化信息类型下拉列表的值
			angular.forEach(EnumType.msgType, function(i) {
				if ($scope.msgSendDefDts.msgType === i.value) {
					$scope.msgSendDefDts.msgType = i;
					}
			});
			angular.forEach(EnumType.loopType, function(i) {
				if ($scope.msgSendDefDts.loopType === i.value) {
					$scope.msgSendDefDts.loopType = i;
					}
			});
			angular.forEach(EnumType.msg_order, function(i) {
				if ($scope.msgSendDefDts.msgOrder === i.value) {
					$scope.msgSendDefDts.msgOrder = i;
					}
			});
            if($scope.msgSendDefDts.sendWay == "0"){
                //alert("radioOptions.direct");               
                $scope.radioOptions.select = "direct";
            }
            if($scope.msgSendDefDts.sendWay == "1"){
                //alert("radioOptions.frequency");
                $scope.radioOptions.select = "frequency";
            }
            //radio 发送类型
            if($scope.msgSendDefDts.loopFlag =="0"){
            	$scope.radioSendTyp.select = "norecycle";
            }
            if($scope.msgSendDefDts.loopFlag =="1"){
                //alert("radioSendTyp.isrecycle");
            	$scope.radioSendTyp.select = "isrecycle";
            }
            var sendChannelParams = $scope.msgSendDefDts.sendChannel.split("|");
            angular.forEach(sendChannelParams, function(i) {
  				if (i == EnumType.sendChannel.message.value) {// 01
   			        $scope.options.duanxin = true;
   					}
   				if (i == EnumType.sendChannel.WeChat.value) {// 02
   			        $scope.options.weixin = true;
   					}
   				if (i == EnumType.sendChannel.email.value) {// 03
   			        $scope.options.youjian = true;
   					}
   				if (i == EnumType.sendChannel.information.value) {// 05
   			        $scope.options.zhanneixin = true;
   					}
   				if (i == EnumType.sendChannel.app.value) {// 04
   			        $scope.options.app = true;
   					}
   			});
			var effectDt = new Date($scope.msgSendDefDts.fixTime);
			$scope.msgSendDefDts.fixDay = effectDt;
			$scope.msgSendDefDts.fixHour = effectDt;
            ...
            $scope.searchTemplate = function() {
                ...
                opts.method = 'GET';
                opts.params = {"tplNo" : $scope.msgSendDefDts.templateCode};
                HttpService.linkHttp(opts).then(function(response) {
                    console.log("请求成功");
                    console.log(response);
                    $scope.msgSendDefDts.templateContent = response.data.list[0].tplCont;
                });
            }

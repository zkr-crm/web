(function () {
  ...
  /** @ngInject */
  function selectStaticCustListCtrl($scope, $http, HttpService,$uibModalInstance, Alert, EnumType, $rootScope) {
      // 用户对象
      $scope.msgTpl = {};
      // 查询条件对象
      $scope.searchObj = {
      ...
      $scope.queryStaticGroupOpts = {
        ...
        success:function(response){
          $scope.RowCollection = response.data.list;
            $scope.smartTableData = response.data.list;
            angular.forEach($scope.smartTableData, function(
                i) {
                if(i.establishTime!=null && i.establishTime.length>=10) {
                    i.establishTime = i.establishTime.substring(0, 10);
                }
            });
        }
      }

...
//        			});   
                    ...
                    $scope.chooseType = function () {
                        if ($scope.checkedRow.length >= 1) {
                            $scope.chooseTypeModal = $uibModal
                                .open({
                                    animation : true,
                                    backdrop : 'static',
                                    templateUrl : 'app/pages/customer/custQuery/custPerQuery/chooseType.html',
                                    size : 'midle-small',
                                    controller : 'addCustGroupCtrl',
                                    scope : $scope,
                                    resolve : {
                    ...
                    $scope.search = function(page){
                        $scope.queryOpts.noDefaultQuery = false
                        ...
                        // var page = page || '1';
                        var isEmpty=true;
                        for(var i in $scope.queryOpts.params){
                            if(i!=null&&i!='sys'&&$scope.queryOpts.params[i]!=null&&$scope.queryOpts.params[i]!=''){
                                isEmpty=false;
                            }
                        }
                        if(isEmpty){
                            $scope.queryOpts.url = '/crm/ecif/cust/getPerCustListByRole';
                        }else{
                    ...
                    var init = function () {
                        ...
                        HttpService.linkHttp(initOpts).then(function(result) {
                            if(!(result.data===undefined)){
                                ...
                                $scope.queryOpts.noDefaultQuery = true
                                $scope.queryOpts.url = '/crm/query/custquery/getPerCustListByRole';
